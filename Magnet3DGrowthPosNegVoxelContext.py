...
mComp1 = rs.GetObject("select mesh component for zero connect", 32)
mComp2 = rs.GetObject("select mesh component for single connect", 32)
mComp3 = rs.GetObject("select mesh component for double straight connect", 32)
mComp4 = rs.GetObject("select mesh component for double L connect", 32)
mComp5 = rs.GetObject("select mesh component for triple connect", 32)
mComp6 = rs.GetObject("select mesh component for quad connect", 32)
    ...
    for j in range(len(grid[i])):
        for k in range(len(grid[i][j])):
            if grid[i][j][k] != 0:
                ...
                tempObj = rs.CopyObject(mCompList[grid[i][j][k]-1])
